...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Proses upload bukti aduan
    $uploadDir = '../uploads/';  // Lokasi folder untuk menyimpan bukti bayar
    $uploadFile = $uploadDir . basename($_FILES['bukti_aduan']['name']);

    if (move_uploaded_file($_FILES['bukti_aduan']['tmp_name'], $uploadFile)) {
        ...
        $query = "INSERT INTO pengaduan (nama_pelapor, user_id, no_rumah, no_telepon, tanggal, kategori_masalah, deskripsi_masalah, status, bukti_aduan) 
        		  VALUES ('$nama_pelapor', '$user_id', '$no_rumah', '$no_telepon', '$tanggal', '$kategori_masalah', '$deskripsi_masalah', 'belum dicek', '$uploadFile')";
        ...
        echo "Gagal mengunggah bukti aduan.";
    }
} 
